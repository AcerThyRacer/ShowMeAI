...
import { Bot, Menu, X, Heart, Keyboard } from 'lucide-react';
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl bg-[var(--bg-color)]/80 border-b border-[var(--accent-color)]/20" role="navigation" aria-label="Main navigation">
      <div className="px-4 flex items-center h-16">
        {/* Left: Logo */}
        <Link to="/" className="flex items-center gap-2 font-bold text-xl group shrink-0 mr-4">
          ...
          <div className="flex items-center gap-0.5 overflow-x-auto scrollbar-hide">
            {links.map((link) => (
              <Link
                key={link.to}
                to={link.to}
                className={`px-2 py-1.5 rounded-lg text-[13px] font-medium transition-all whitespace-nowrap ${isActive(link.to)
        ...
        <div className="hidden lg:flex items-center gap-2 shrink-0 ml-4">
          ...
          <button
            ...
            className="w-9 h-9 rounded-full flex items-center justify-center hover:bg-[var(--accent-color)]/10 border border-[var(--accent-color)]/20 transition-colors"
